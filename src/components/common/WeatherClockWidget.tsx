...
function getWeatherDescription(code: number, isDay: boolean): { label: string; iconName: string } {
  ...
  if (code === 3) {
    return { label: 'Bulutli havo', iconName: 'cloud' };
  }
  if (code === 45 || code === 48) {
    return { label: 'Tumanli', iconName: 'fog' };
  }
  if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82)) {
    return { label: 'Yomg‘irli havo', iconName: 'rain' };
  }
  if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) {
    return { label: 'Qor yog‘moqda', iconName: 'snow' };
  }
  if (code >= 95 && code <= 99) {
    return { label: 'Momaqaldiroq', iconName: 'lightning' };
  }
  return { label: isDay ? 'Quyoshli' : 'Ochiq havo', iconName: isDay ? 'sun' : 'moon' };
}
...
export const WeatherClockWidget: React.FC = () => {
  ...
  const { label: weatherLabel, iconName: weatherIconName } = getWeatherDescription(weather.weatherCode, weather.isDay);
  const formattedTemp = weather.temp > 0 ? `+${weather.temp}°` : `${weather.temp}°`;

  return (
    <div className="relative inline-block" ref={popoverRef}>
      {/* MAIN LUXURY WIDGET PILL WITH 3D FLUENCY ICONS */}
      <button
      ...
      >
        {/* Weather 3D Icon section */}
        <div className="weather-section flex items-center gap-1.5 shrink-0">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-amber-500/10 border border-amber-500/25 flex items-center justify-center group-hover:scale-105 transition-transform shadow-inner shrink-0 p-0.5">
            <Icon3D name={weatherIconName} size={22} />
          </div>
        ...
        <div className="clock-section flex items-center gap-1.5 shrink-0">
          <Icon3D name="clock" size={16} className="hidden xs:inline shrink-0" />
        ...
        <div className="date-section hidden sm:flex items-center gap-1.5 shrink-0">
          ...
          <Icon3D name="calendar" size={16} className="hidden md:inline shrink-0" />
      ...
      {/* DETAILED POPOVER CARD WITH 3D FLUENCY ICONS */}
      {isOpenModal && (
        <div className="weather-clock-popover absolute right-0 mt-2.5 w-[calc(100vw-2rem)] max-w-sm sm:w-96 rounded-3xl bg-[#140E0A]/95 border border-amber-500/30 shadow-[0_20px_50px_rgba(0,0,0,0.85)] backdrop-blur-2xl p-5 z-50 animate-fade-in text-stone-100">
          {/* Header with 3D Compass Icon */}
          <div className="flex items-center justify-between pb-3.5 border-b border-amber-950/80">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center shadow-md p-1 shrink-0">
                <Icon3D name="compass" size={28} />
              </div>
              <div>
          ...
          {/* Weather Details Card with 3D Icons */}
          <div className="p-4 rounded-2xl bg-[#1A120B]/80 border border-amber-950/90 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center shadow-md p-1 shrink-0">
                  <Icon3D name={weatherIconName} size={36} />
                </div>
                <div>
              ...
              <button
                ...
                className="p-2 rounded-xl bg-stone-900/80 hover:bg-amber-500/20 text-stone-400 hover:text-amber-300 border border-stone-800 transition-all active:scale-95 shrink-0"
                title="Ob-havoni yangilash"
              >
                <Icon3D name="refresh" size={20} className={isLoadingWeather ? 'animate-spin' : ''} />
              </button>
            </div>

            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-stone-800/70 text-xs">
              <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-black/30 border border-stone-900">
                <Icon3D name="location" size={22} className="shrink-0" />
              ...
              <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-black/30 border border-stone-900">
                <Icon3D name="wind" size={22} className="shrink-0" />
                <div>
